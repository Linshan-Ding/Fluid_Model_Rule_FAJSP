#AC算法的基础框架代码，包含策略网络、价值网络、AC Agent和轮训训练主程序
import os
import sys
import signal
import torch
...
# 初始化Visdom
viz = visdom.Visdom(env='FAJSP_AC_Training')
...
class ACAgent:
    def __init__(self, action_feature_dim, combined_dim, mlp_value_dim,
                 policy_lr=1e-4, value_lr=1e-4, gamma=0.99):
        self.device = device
        self.gamma = gamma
    ...
    def take_action(self, matrix_states, candidate_actions, candidate_actions_features):
        ...
        return selected_action, action_idx, log_prob

    def store_transition(self, state, action, action_idx, log_prob,
    ...
    def update(self):
        ...
        for s in states:
            state_values.append(self.value_net(s))
        for ns in next_states:
            next_state_values.append(self.value_net(ns))

        state_values = torch.cat(state_values).to(self.device)
        next_state_values = torch.cat(next_state_values).to(self.device)

        # 时序差分目标
        td_target = rewards + self.gamma * next_state_values * (1 - dones)
        td_delta = td_target - state_values  # 时序差分误差

        # 重新计算对数概率（这是必须的，因为策略网络参数可能已更新）
        log_probs = []
        ...
        # 策略损失：-log_prob * td_delta
        actor_loss = torch.mean(-log_probs_tensor * td_delta.detach())

        # 价值损失：MSE
        critic_loss = torch.mean(F.mse_loss(state_values, td_target.detach()))
...
class InstanceTrainer:
    ...
    def train_one_episode(self):
        ...
        while True:
            ...
            # 选择动作
            action, action_idx, log_prob = self.agent.take_action(
    ...
    def save_interrupt_results(self, round_num=None):
        ...
        interrupt_path = f"ac_fajsp_model_{config_str}.pth"
    ...
    def save_results(self):
        config_str = self.config_str
        save_path = f"ac_fajsp_model_{config_str}.pth"
...
def train_round_robin(data_configs, total_episodes_per_instance=10000):
    ...
    # 创建平均完工时间的Visdom窗口（与DQN一致）
    avg_ct_win = viz.line(
        X=np.array([0]),
        Y=np.array([0]),
        opts=dict(
            title='Round Average Completion Time (AC)',
            xlabel='Round',
            ylabel='Average CT',
            env='FAJSP_AC_Training',
            showlegend=False
    ...
    try:
        while True:
            ...
            if interrupt_flag:
                ...
                # 保存全局中断状态
                if global_agent:
                    interrupt_global_path = "ac_fajsp_model_global.pth"
                    torch.save({
            ...
            if round_instance_count > 0:
                ...
                # 更新平均CT Visdom曲线（与DQN一致）
                viz.line(
                    X=np.array([round_num]),
                    Y=np.array([round_avg_ct]),
                    win=avg_ct_win,
                    update='append',
                    opts=dict(title='Round Average Completion Time (AC)', xlabel='Round', ylabel='Average CT')
                )
    ...
    if not interrupt_flag:
        ...
        if global_agent and global_agent.global_best_policy_state is not None:
            global_best_path = "ac_fajsp_global_best.pth"
            torch.save({
                'global_best_policy_state': global_agent.global_best_policy_state,
                'global_best_value_state': global_agent.global_best_value_state,
                'global_best_avg_ct': global_agent.global_best_avg_ct,
...
if __name__ == "__main__":
    ...
    # 测试用的小列表
    test_list = [[4, 2, 2, 4, 2, 4]]

    print("AC算法轮训训练开始")
    print("按 Ctrl+F2 中断训练")

    # 开始轮训训练
    trainers = train_round_robin(test_list, total_episodes_per_instance=10000000)